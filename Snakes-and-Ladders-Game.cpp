...
char faces[dice_Faces]={'A','B','C','D','E','F'};
char grid[N][M];
...
bool check_valid_face(char i) {
	for (int col = 0 ; col < dice_Faces ; col++){
    if(faces[col] == i){
			return true;
		}
	}
...
void move_player(int player, int moves) {
  if(player_position[player] + moves > 100){
...
int get_entity_value(int p) {
  for (int row = 0 ; row < n_components ; row++){
    if(ladders[row][0] == p){
      return ladders[row][1]-p;
    }
    else if(snakes[row][1] == p){
      return snakes[row][0] - snakes[row][1];
    }
  }
  return 0;
}
